...
const fetchAttachmentsByQuoteIds = async (quoteIds = []) => {
  ...
  return rows.reduce((acc, row) => {
    ...
    acc[row.quote_id].push(mapAttachment(row))
    return acc
  }, {})
}
...
export const getAllCotizaciones = catchAsync(async (req, res) => {
  ...
  const attachmentsByQuote = await fetchAttachmentsByQuoteIds(quotes.map((quote) => quote.id))

  const quotesWithAttachments = quotes.map((quote) => ({
    ...quote,
    attachments: attachmentsByQuote[quote.id] || []
  }))
  ...
  const totalCotizaciones = parseInt(countResult.rows[0].count, 10)
  const totalPages = Math.max(Math.ceil(totalCotizaciones / parsedLimit), 1)

  res.json({
    success: true,
    message: 'Cotizaciones obtenidas exitosamente',
    data: quotesWithAttachments,
    pagination: {
...
export const getCotizacionById = catchAsync(async (req, res) => {
  ...
  const attachmentsByQuote = await fetchAttachmentsByQuoteIds([quote.id])
...
export const createCotizacion = catchAsync(async (req, res) => {
  const { user_id, name, email, phone, message, source } = req.body
  ...
  const result = await query(
    ...
    [user_id || null, name, email, phone || null, message, 'pendiente', source || 'admin_panel']
  )
...
export const updateCotizacion = catchAsync(async (req, res) => {
  ...
  const existingCotizacion = await query('SELECT id FROM quotes WHERE id = $1', [id])
  ...
  const attachmentsByQuote = await fetchAttachmentsByQuoteIds([result.rows[0].id])
  ...
  res.json({
    ...
    data: {
      ...result.rows[0],
      attachments: attachmentsByQuote[result.rows[0].id] || []
    }
  })
})
...
export const getCotizacionStats = catchAsync(async (req, res) => {
  ...
  res.json({
    ...
    data: {
      totalCotizaciones: parseInt(totalCotizaciones.rows[0].total, 10),
      cotizacionesByStatus: cotizacionesByStatus.rows,
      recentCotizaciones: parseInt(recentCotizaciones.rows[0].count, 10)
    }
  })
})
